...
def _fill_missing_titles(df):
    logger.info('Filing missing titles')
...
def _tokenize_column(df, column_name):
    ...
    n_tokens = (df
           ...
           .apply(lambda tokens: list(filter(lambda token: token.isalpha(),tokens))) # quita los alphanumericos
           .apply(lambda tokens: list(map(lambda token: token.lower(),tokens))) #todo a minuscula
           .apply(lambda word_list: list(filter(lambda word: word not in stop_words,word_list))) #quita las stop words
...
def _save_data(df, filename):
    ...
    logger.info(f'Saving data al location {clean_filename}')
